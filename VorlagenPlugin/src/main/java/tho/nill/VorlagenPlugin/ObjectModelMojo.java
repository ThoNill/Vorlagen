...
import org.nill.vorlagen.object.ClassNames2ModellGenerator;
...
@Mojo(name = "generateFromObjectModel", defaultPhase = LifecyclePhase.GENERATE_SOURCES)
public class ObjectModelMojo extends MojoWithFileChecks {
	...
	public void execute() throws MojoExecutionException {
		...
		try {
			...
			List<String> compilerArgs = new ArrayList<>(Arrays.asList("-source", source, "-target", target));
			List<String> sourcePaths = new ArrayList<>();
			sourcePaths.add(modellDir);
			sourcePaths.add(localRepository + "/tho/nill/Vorlagen-compiler/" + vorlagenVersion + "/Vorlagen-compiler-"
					+ vorlagenVersion + "-sources.jar");
			sourcePaths.add(localRepository + "/tho/nill/Vorlagen-vorlagen/" + vorlagenVersion + "/Vorlagen-vorlagen-"
					+ vorlagenVersion + "-sources.jar");
			...
			if (modelClasses != null && modelClasses.length > 0) {
				new ClassNames2ModellGenerator(templateDir, outputDir, Arrays.asList(modelClasses), sourcePaths,
						compilerArgs, converter, connector, modellDir).erzeugeAusgabe();
			} else {
	...
	private ConverterVerzeichnis createConverterVerzeichnis()
